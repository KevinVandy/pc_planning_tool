...
/*
	This is the development file. The Header file actually references the min version of this code for faster loading time
	By Kevin Van Cott
...
var numMonitors = parseInt($("#numMonitors").val());
...
function changenumMonitors() {
	$("#numMonitors").val(numMonitors);
}
...
function drawMonitor(i) {
	if (getSize(i) > 3 && getSize(i) < 100 && getHorRes(i) > 100 && getHorRes(i) < 50000 && getVerRes(i) > 100 && getVerRes(i) < 50000) {
        ...
        if(isNaN(bezelWidth) || bezelWidth == undefined) bezelWidth = 1.00;
...
function removeMonitor() {
	if (numMonitors > 0) {
		$("#monitorBox" + numMonitors--).fadeOut(200);
		displayTotalNumPixels();
        displayTotalWidth();
        displayTotalArea();
        displayTotalMonitorsCost();
...
function calculateTotalMonitorsCost() {
    var totalCost = 0.00;
    for (var i = 1; i <= numMonitors; i++) {
...
function displayTotalMonitorsCost() {
    $("#totalCost").html("$ " + calculateTotalMonitorsCost().toFixed(2)); //totals section
    $(".monitorsTotalCostHidden").val(calculateTotalMonitorsCost()); //hidden input
}

function calculateTotalSetupCost() {
    var totalCost = 0.00;
    
    var PCPartsCost = parseFloat($(".PCPartsTotalCostHidden").val());
    if(isNaN(PCPartsCost)) PCPartsCost = 0;
    totalCost += PCPartsCost;
    
    totalCost += calculateTotalMonitorsCost();
...
function searchMonitor(i) { // updates the search url for each monitor
	...
	if (getResponseTime(i) != null && getResponseTime(i) != undefined && getResponseTime(i) != "" && getResponseTime(i) != 0) searchURL += "+" + getResponseTime(i) + "ms";
